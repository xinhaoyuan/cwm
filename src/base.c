...
setup(void)
{
    ...
    for (i = 0; i < screen_count; ++ i)
    {
        ...
        for (i = 0; i < len; i ++)
        {
            ...
            {
                hook_client_created(client_attach(children[i]));
            }
...
xcb_event_map_request(xcb_generic_event_t *e)
{
    ...
    case ROLE_INIT:
    {
        hook_client_created(client_attach(map_request->window));
        break;
    }

    case ROLE_CLIENT_ORIG:
        hook_client_orig_mapped((client_t)node->link);
        break;
...
xcb_event_unmap_notify(xcb_generic_event_t *e)
{
    ...
    {
        hook_client_orig_unmapped((client_t)node->link);
    }
}
...
xcb_event_reparent_notify(xcb_generic_event_t *e)
{
    ...
    {
        hook_client_before_destroy(client);
        client_free(client);
    }
